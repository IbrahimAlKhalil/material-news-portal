...
export default withStyles(styles)(
    class extends React.Component<Props, State> {
        springConfig: SpringConfig = {duration: '0.8s', delay: '0s', easeFunction: 'ease-in-out'};
        animationId: number = 0;
        timeoutId: number = 0;
        slideCount: number = this.props.slides.length - 1;
        ...
        render() {
            ...
            return (
                <Paper component="section" className={`${classes.root} content-block spacing-right-md spacing-left-md`}>
                    <SwipeableViews axis="x" index={this.state.index} autoPlay springConfig={this.springConfig}>
                        {
                            slides.map((slide, index) => (
                                <div key={`sl-${index}`} className={classes.container}>
                                    <img src={slide.image} alt={slide.alt ? slide.alt : slide.title}
                                         className={classes.img}/>
                                    <div className={classes.excerpt}>
                                        <h2>
                                            <Link className={classes.text}
                                                  to={`/${slide.slug}`}>{slide.title}</Link>
                                        </h2>
...
export interface Slide extends PostCommon {
    image: string
    excerpt: string
    alt: string
}