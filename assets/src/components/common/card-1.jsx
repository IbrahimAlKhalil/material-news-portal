...
export default withStyles(styles)(class extends React.Component {
    render() {
        ...
        return (<Card className={`${props.className} ${classes.card} flex justify-content-between`}>
                    <CardContent>
                        <Typography variant={props.bigCard ? 'h1' : 'h2'} className={classes.heading}>
                        ...
                        <div className={`${classes.categoryContainer} flex align-items-center`}>
                            {post.categories.map((category, index) => (<Button key={`c-${index}`} className={classes.category} variant="outlined" size="small" color="primary" component={({ innerRef, ...props }) => <Link to={`/category/${category.slug}/`} {...props}/>}>
                                        {category.name}
                                    </Button>))}

                            <Watch color="action"/>
                            &nbsp;
                            <a href={`/${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`} className="link-primary">
                                <time dateTime={''}>{postDate(post.date)}</time>
                            </a>
                        </div>
                        <img src={post.image} alt={post.alt ? post.alt : post.title} className={classes.img + (props.bigCard ? ' ' + classes.bigCardImg : '')}/>
                    ...
                    <CardActions className="flex justify-content-between">
                        ...
                        <Button component={({ innerRef, ...props }) => <Link {...props} to={`/${post.slug}`}/>} color="primary" variant="outlined">আরও পড়ুন</Button>
                    </CardActions>
                </Card>);
    }
});
