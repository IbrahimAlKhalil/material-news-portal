...
export default withStyles(styles)(class extends React.Component<Props> {
    render() {
        ...
        const CommentTemplate: FunctionComponent<{ comment: CommentType, sub?: boolean }> = ({comment, sub}) => (
            <div className={sub ? classes.subComment : ''}>
                <div className="flex">
                    <div>
        ...
        return (
            <div className={classes.wrapper} id={`comment-${comment.id}`}>
                <CommentTemplate comment={comment}/>
                {comment.children.length ? comment.children.map((subComment, index) => <CommentTemplate
                    comment={subComment} key={`s-${index}`} sub/>) : null}
