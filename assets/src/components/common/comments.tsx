...
export default withStyles(styles)(
    class extends React.Component<Props, State> {
        ...
        render() {
            ...
            if (!data) {
                return <Fallback height="200px" suspense={false} progress={true}
                                 className="spacing-right-md spacing-left-md"/>
            }
            ...
            return (
                <section className={`spacing-right-md spacing-left-md ${className}`}>
                    {
                        data.comments.length ? <Typography
                            className={classes.commentSectionTitleLeft}>{enNumberToBn(data.comments.length.toString())} টি
                            মন্তব্য</Typography> : null
                    }
                    {
                        data.comments.map((comment, index) => <Comment comment={comment} key={`c-${index}`}
